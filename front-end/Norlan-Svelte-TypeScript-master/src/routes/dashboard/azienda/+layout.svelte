<script lang="ts">
		...
		if (showNotifiche) {
			...
			if (session) {
				try {
					...
					console.error("Si è verificato un problema tecnico durante il caricamento dell'elenco notifiche:", error);
				} finally {
	...
	async function handleLeggiNotifica(idNotifica: number) {
		try {
			...
			console.error("Impossibile aggiornare lo stato della notifica (lettura fallita):", error);
		}
	}
