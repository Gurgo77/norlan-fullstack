<script lang="ts">
	...
	interface DipendenteFormazione {
		...
		ruolo: string;
		corsi: CorsoStato[];
		tuttiIdCorsiIscritto: number[]; // Aggiunto per tracciamento DB reale
	}
	...
	onMount(async () => {
		...
		try {
			...
			const promises = lavoratoriRaw.map(async (l) => {
				...
				// Salviamo tutti gli ID per il controllo duplicati nel modale
	...
	// --- LOGICA REATTIVA PER FILTRARE CORSI NEL MODALE ---
	const corsiSelezionabili = $derived.by(() => {
		...
		// Ritorna solo i corsi dove il dipendente non è già presente nel DB[cite: 1]
	...
	async function confermaIscrizione() {
		...
		try {
			...
			if (corsoScelto && dipendenteScelto) {
				...
				// Aggiorniamo anche la lista ID per bloccare reinserimenti immediati
		...
		<div class="bg-white w-full max-w-lg rounded-3xl shadow-2xl flex flex-col overflow-hidden relative" in:scale>
			...
			<div class="p-8 flex-1 bg-gray-50/50 space-y-6">
				...
				<div class="space-y-2">
					...
					<!-- FILTRO DI SICUREZZA APPLICATO QUI[cite: 1] -->
